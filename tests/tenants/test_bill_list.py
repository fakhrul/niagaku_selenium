...
from pages.tenants.bill_list_page import BillListPage
import os
...
def test_open_bill_list(setup):
    driver = setup
    bill_list_page = BillListPage(driver)

    # Navigate to the Quotation List
    bill_list_page.navigate_to(APP_URL + "/pages/login")
    bill_list_page.input_text_by_test_id(("usernameInput"), "tenant1@niagaku.com")
    bill_list_page.input_text_by_test_id(("passwordInput"), "qwe123")
    bill_list_page.click((By.CSS_SELECTOR, ".btn-success"))

    # Navigate to Quotation List and Add Quotation
    bill_list_page.navigate_to_bill_list()
    ...
    assert "Bill List added successfully"
